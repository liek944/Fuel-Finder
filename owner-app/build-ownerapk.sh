...
if [ -d "../frontend/src/components/owner" ]; then
  # Automatically copy the latest files (force overwrite if read-only)
  cp -f ../frontend/src/api/ownerApi.ts src/api/
  cp -f ../frontend/src/components/owner/* src/components/
  ...
  for file in src/components/*.tsx; do
    if [ -f "$file" ]; then
      ...
      # Add a large warning to the top of the file to prevent accidental edits
      sed -i '1s|^|// 🛑 WARNING: DO NOT EDIT THIS FILE DIRECTLY!\n// This file is auto-copied from frontend/src/components/owner/ upon build.\n// Any changes made here will be OVERWRITTEN.\n\n|' "$file"
      
      # Make the file read-only so IDEs will block editing
      chmod 444 "$file"
    fi
  done
else
